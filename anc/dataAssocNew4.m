...
%                     end
        ...
        try
            uprimal_uncertain = intlinprog([],1:len_uncertain,[],[],sparse(A_uncertain),b_uncertain,zeros(len_uncertain,1),ones(len_uncertain,1),[],options);
        catch
            uprimal_uncertain = [];
        end
        if isempty(uprimal_uncertain)
            bestPrimalCosthat = bestPrimalCost;
        else
